...
def main():
    haiku = loadHaiku('train.txt')
    exceptions = cmudictMissing(haiku)
    build_dict = input("\nBuild an exceptions dictionary? (y/n) \n")
    if build_dict.lower() == 'n':
        sys.exit()
    else:
        missing_words_dict = makeExceptionsDict(exceptions)
        saveExceptions(missing_words_dict)


def loadHaiku(filename):
    with open(filename) as in_file:
        haiku = set(in_file.read().replace('-', ' ').split())
        return haiku


def cmudictMissing(word_set):
    exceptions = set()
...
def makeExceptionsDict(exceptions_set):
    missing_words = {}
...
def saveExceptions(missing_words):
